...
public class ExperienceManager {
	...
	public void createBottle(Player player, int experience) {
		...
		int originalExp = Math
				.round(getExpToLevel(originalLevel) + getExpForProgressToNextLevel(player.getExp(), originalLevel));

		// Prevent EXP dupe
		player.setLevel(0);
		player.setTotalExperience(0);

		// Calculate player's new experience. -1 is always subtracted to compensate
